...
for i = 1:m
    a_1 = [ones(size(X(i,:)), 1), X(i,:)];
    a_2 = [ones(size(a_1),1), sigmoid(a_1*Theta1')];
    a_3 = sigmoid(a_2*Theta2');
    hTheta = a_3;
    J = J + (1/m)*((log(hTheta))*(-Y(:,i)) - log(1-hTheta)*(1-Y(:,i)));    
end;

for j = 1:(size(Theta1))(1);
    for k = 2:(size(Theta1))(2);
        reg1 = reg1 + Theta1(j,k)'*Theta1(j,k);
    end;
end;

for j = 1:(size(Theta2))(1);
    for k = 2:(size(Theta2))(2);
        reg2 = reg2 + Theta2(j,k)'*Theta2(j,k);
    end;
end;

regularizer = regScale* (reg1 + reg2);
